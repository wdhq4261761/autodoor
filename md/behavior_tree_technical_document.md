...
系统根据识别语言自动选择预处理模式：
- **中文预处理**（chi_sim/chi_tra）：针对中文文字特点优化
- **标准预处理**（其他语言）：通用图像预处理

**中文预处理流程**（chi_sim/chi_tra）：
```
原始截图
    │
    ▼
┌─────────────┐
│  图像放大    │  小图像（宽或高<300px）放大2.5倍
└──────┬──────┘
       │
       ▼
┌─────────────┐
...
OCR关键词识别支持自动尝试多种PSM（Page Segmentation Mode）模式，提高识别成功率：
```
PSM模式尝试顺序：
1. PSM 7：将图像视为单行文本
2. PSM 6：将图像视为单一均匀文本块
3. PSM 11：稀疏文本，寻找尽可能多的文本
...
变量判断节点支持自动类型推断，比较值会根据变量的实际类型自动转换：

| 变量实际类型 | 比较值转换规则 |
|------------|--------------|
| bool | 字符串 "true"/"1"/"yes" → True，"false"/"0"/"no" → False |
| int/float | 字符串自动转换为数字（支持整数和浮点数） |
| str | 保持原样 |
| None | 保持原样 |
...
变量 count = 10 (int类型)
比较值 = "5" (字符串)
结果：自动将 "5" 转换为 5 (int)，执行 10 > 5 → 成功

变量 enabled = True (bool类型)
比较值 = "true" (字符串)
结果：自动将 "true" 转换为 True，执行 True == True → 成功
```
